	...
	function doServerTest(){
		...
		if (version==-1 || version>=9.0){
			...
			for (var i=0; i<form.elements.length; i++){
				if (i>0) params += "&";
                var value = form.elements[i].value.replace("&", "%26");
                if (!value)
                    value = form.elements[i].placeholder;
				params += form.elements[i].id+"="+value;
			}
...
<fieldset>
	...
	<table>
		...
		<tr class="pgsql"><td>Initial Database:</td><td><input type=text id="db" placeholder=""></td></tr>
		...
		<tr class="sqlite"><td>Database File:</td><td><input type=text id="dbfile" placeholder=""></td></tr>
